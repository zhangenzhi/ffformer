...
def forainetv2_data_prep(root_path, info_prefix, out_dir, workers):
    """Prepare the info file for ForAINetV2 dataset.
    ...
    info_train_path = osp.join(
        out_dir, f'{info_prefix}_oneformer3d_infos_train.pkl')
    info_val_path = osp.join(
        out_dir, f'{info_prefix}_oneformer3d_infos_val.pkl')
    info_test_path = osp.join(
        out_dir, f'{info_prefix}_oneformer3d_infos_test.pkl')
...
parser.add_argument(
    'dataset', metavar='forainetv2', help='name of the dataset')
parser.add_argument(
    '--root-path',
    type=str,
