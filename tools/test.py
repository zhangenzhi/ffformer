...
def main():
    ...
    if key_to_fix not in checkpoint:
        raise KeyError(
            f"Could not find a state dictionary ('state_dict') "
            f"in the checkpoint: {args.checkpoint}")
    ...
    with tempfile.NamedTemporaryFile(
            delete=False, suffix='.pth') as tmp_checkpoint_file:
        torch.save(checkpoint, tmp_checkpoint_file.name)
        cfg.load_from = tmp_checkpoint_file.name
    print(f'Spconv checkpoint fix applied. '
          f'Using temporary checkpoint: {cfg.load_from}')
