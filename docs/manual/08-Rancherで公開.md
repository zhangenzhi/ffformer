...
| 资源 | 名字 | 说明 |
|---|---|---|
| Namespace | `c30636-default` | 我们组的区画(已存在,和 dify/ollama 等共用) |
...
3. 确认 **`c30636-default`** 已存在;若要全新区画才点 **Create Namespace**。

> **Namespace** = 同一栋楼(集群)里分出来的"房间",分开管理不打架。我们统一用 `c30636-default`。
...
2. Namespace 选 `c30636-default`。
3. 仓库地址 `docker.io`,填 Docker Hub 用户名 + **Access Token**(不是登录密码,在 Docker Hub → Account Settings → Security 里生成)。
...
```bash
kubectl -n c30636-default create secret generic ffformer-hpc-ssh \
  --from-file=key=/path/to/hpc_ed25519_private_key
```

Deployment 里通过 `HPC_KEY_PATH=/secrets/hpc/key` 引用(见 [07](07-K8sマニフェスト.md))。超算侧 `authorized_keys` 已限制来源 `from="172.31.232.*",no-pty`(pod 的 SNAT 网段)。
...
对外访问链路:
```
用户 → 公网 133.50.39.16:443 →(中心 NAT)→ 中间IP 172.31.229.16
     → MetalLB LoadBalancer(ffformer-public)→ ffformer-tls(nginx 终结 HTTPS)→ ffformer-infer-base:8000
```

`ffformer-public` Service 的关键:用注解 **`metallb.universe.tf/loadBalancerIPs: 172.31.229.16`** 让 MetalLB 把中间 IP 绑上;`selector` 指向 `app=ffformer-tls`;端口 443→8443。

> 公网 IP / 中间 IP 是向中心「サービス公開申請」申请来的(放行来源:校园网 `133.87.0.0/16`、`133.50.0.0/16`)。若新 Service 外部 IP 一直 `<pending>`、MetalLB 报 `not allowed in config`,说明该中间 IP 还没进 MetalLB 池,需要中心配。
...
命令行等价:
```bash
kubectl -n c30636-default get deploy ffformer-infer-base
kubectl -n c30636-default get pods -l app=ffformer-infer-base -o wide
kubectl -n c30636-default logs deploy/ffformer-infer-base | tail
```
...
- ☐ `ffformer-public` 有外部 IP `172.31.229.16`(不是 `<pending>`)
- ☐ 浏览器开 `https://j-peaks-forestformer3d.hucc.hokudai.ac.jp/`(或 `https://133.50.39.16/`)能看到登录页
