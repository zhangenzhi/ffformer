...
def FastFocalLoss(out, target, ind, mask):
    '''
    Custom Focal Loss for 3D voxel-based tasks.
    Arguments:
      out, target: B x N x 1 (prediction scores for each voxel)
      ind: indices of top k positive samples
      mask: B x N (mask to indicate which points are valid)
    '''
    mask = mask.float()  # mask indicates which points are valid

    # Apply clamping to prevent log(0) and ensure numerical stability
    out = torch.clamp(out, min=1e-6, max=1-1e-6)  # Clamp output to avoid extreme values
    target = torch.clamp(target, min=1e-6, max=1-1e-6)  # Clamp target to avoid extreme values

    gt = torch.pow(1 - target, 6)  # apply weighting to negative samples
    neg_loss = torch.log(1 - out) * torch.pow(out, 3) * gt
    neg_loss = neg_loss.sum()

    # Gather predictions for the positive samples using ind
    pos_pred = out[ind].squeeze(1)  # Get predictions for positive samples
    pos_pred = torch.clamp(pos_pred, min=1e-6, max=1-1e-6)  # Ensure pos_pred is in a stable range

    pos_mask = mask[ind]  # Mask for the positive samples
    num_pos = pos_mask.sum()  # Number of positive samples

    pos_loss = torch.log(pos_pred) * torch.pow(1 - pos_pred, 3) * pos_mask  # Compute positive loss
    pos_loss = pos_loss.sum()

    if num_pos == 0:
        return -neg_loss
    return - (pos_loss + neg_loss) / max(num_pos, 1e-6)  # Ensure no division by zero
